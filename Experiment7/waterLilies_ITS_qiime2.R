...
count<-read.delim("/Users/briannepalmer/Library/CloudStorage/OneDrive-Personal/LeafFossilization/Dataframes_to_merge/feature-table_ITS.tsv", header=FALSE, row.names=NULL, stringsAsFactors=FALSE, na.strings = "n/a")

count.sw <-read.delim("/Users/briannepalmer/Library/CloudStorage/OneDrive-Personal/LeafFossilization/PondWater_PondSoil_GinkgoAquarium_ITS/demux/Brianne_SoilWater_ITS/ASVs/feature-table.tsv", header=FALSE, row.names=NULL, stringsAsFactors=FALSE, na.strings = "n/a")
...
colnames(count.sw)<-as.character(unlist(count.sw[2,]))
count1.sw<-count.sw[-c(1,2),]
colnames(count1.sw)[1]<-"Feature.ID"
head(count1.sw[1:2,]) # check dataframe structure
x<-dim(count1.sw)[2];x # number of columns
# Convert class in count1 to numerics
count2.sw<-count1.sw
x<-dim(count2.sw)[2];x # number of columns
count2.sw[2:x] <- lapply(count2.sw[2:x], function(x) as.numeric(as.character(x)))
...
sum(seq_total) # total number of sequences over whole dataset #269179
dim(count2)[1] # total unique OTUs over whole dataset # 631


seq_total<-apply(count2.sw[2:x],2,sum) #number of sequences per sample
OTU_count<-colSums(count2.sw[2:x]>0) # OTUs per sample
OTU_single<-colSums(count2.sw[2:x]==1) # OTUs with only 1 seq
OTU_double<-colSums(count2.sw[2:x]==2) # OTUs that have only 2 seqs
OTU_true<-colSums(count2.sw[2:x]>2) # Number of OTUs with >2 seqs
dataset_info.sw<-data.frame(seq_total,OTU_count,OTU_single,OTU_double,OTU_true)

sum(seq_total) # total number of sequences over whole dataset #52444
dim(count2.sw)[1] # total unique OTUs over whole dataset # 1012
...
rowsum<-apply(count2.sw[2:x],1,sum) # Number of sequences per OTU (for whole dataset)
count.no1.sw = count2.sw[ rowsum>1, ] # Remove rowsums =< 1 (only one sequence for 1 OTU)
dim(count2.sw)[1] - dim(count.no1.sw)[1] # Total number of global singletons removed from data
## Switch values above to remove global doubletons as well!

# Import taxonomy file and join with count data:
taxname<-read.delim("/Users/briannepalmer/Library/CloudStorage/OneDrive-Personal/LeafFossilization/Dataframes_to_merge/taxonomy_ITS.tsv", header=TRUE, row.names=NULL)
library(tidyverse)
taxname <- separate(data = taxname, col = Taxon, into = c("kingdom", "phylum", "class", "order", "family", "genus", "species"), sep = ";")


taxname.sw<-read.delim("/Users/briannepalmer/Library/CloudStorage/OneDrive-Personal/LeafFossilization/PondWater_PondSoil_GinkgoAquarium_ITS/demux/Brianne_SoilWater_ITS/tax_dir/taxonomy.tsv", header=TRUE, row.names=NULL)
taxname.sw <- separate(data = taxname.sw, col = Taxon, into = c("kingdom", "phylum", "class", "order", "family", "genus", "species"), sep = ";")


library(plyr) # Load plyr library into R environment
counts_wtax <-join(count.no1, taxname, by="Feature.ID", type="left", match="first")
counts_wtax.sw <-join(count.no1.sw, taxname.sw, by="Feature.ID", type="left", match="first")
...
counts_wtax$ASV <- c(paste("ASV", seq(c(1:630))))

# Write output table with OTU or ASV cluster information and taxonomy IDs:
write.table(counts_wtax, file="/Users/briannepalmer/Downloads/NymphaeaNuphar_ITSRaw/ASVs/OutputTable_wtax.txt", quote=FALSE, sep="\t", row.names=FALSE)

library(data.table)
metadata<-fread("/Users/briannepalmer/Downloads/NymphaeaNuphar_ITSRaw/sample-metadata.tsv")
...
adonis2(transformed ~ Location, metadata) # 0.048
adonis2(transformed ~ plant, metadata) # 0.16

# pivot 

pivot <- counts_wtax %>% pivot_longer(cols = c(NuBotR1:NyTopR3), names_to = "SampleID")
pivot.phylum <- pivot %>% group_by(SampleID, phylum) %>% summarize(value = sum(value)) %>% mutate(rel = value / sum(value)) 

phylum <- pivot.phylum %>% dplyr::select(SampleID, phylum, rel)
phylum <- phylum %>% pivot_wider(names_from = phylum, values_from = rel)
phylum[c(2:6)] <- sapply(phylum[c(2:6)],as.numeric)
phylum[is.na(phylum)] <- 0

phylum <- as.data.frame(c(metadata, phylum))
phylum.ggplot <- pivot_longer(phylum, cols = p__Ascomycota:NA.,names_to = "phylum", values_to = "count")
